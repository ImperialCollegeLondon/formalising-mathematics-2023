...
begin
  rw ← Sup_empty,
  apply Sup_le,
  rintro b ⟨⟩, -- recall `b ∈ ∅` is defined to mean `false`, and `cases` on a proof of `false` 
end            -- solves the goal
...
begin
  ...
  { rw ← Sup_empty,
    ...
    apply Sup_le,
    rintro _ ⟨⟩, },
  { rintro rfl, -- quicker than `intro h, rw h`
    refl, }, -- `≤` is reflexive
end
...
begin
  ...
  intros b hb,
  apply le_Sup,
  apply hST, -- definitional abuse: S ⊆ T is *defined* to mean `∀ a, a ∈ S → a ∈ T`
  exact hb,
end
