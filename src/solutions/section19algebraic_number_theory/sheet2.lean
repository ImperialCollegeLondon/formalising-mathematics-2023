...
begin
  rw lemma1 at ha ⊢,
  have hb' : is_integral (algebra.adjoin ℤ ({a} : set K)) b,
  { apply is_integral_of_is_scalar_tower hb,
    { exact algebra_int ↥(algebra.adjoin ℤ {a}), },
    { exact is_scalar_tower.subalgebra' ℤ K K (algebra.adjoin ℤ {a}), }, },
  rw lemma1 at hb',
  have foo : ((algebra.adjoin ℤ ({a, b} : set K))) = 
    (algebra.adjoin ↥(algebra.adjoin ℤ ({a} : set K)) ({b} : set K)).restrict_scalars ℤ,
  { apply le_antisymm,
    { rw [algebra.adjoin_le_iff, subalgebra.coe_restrict_scalars],
      rintro x (rfl | hxb),
